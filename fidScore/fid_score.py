...
IMAGE_EXTENSIONS = {'bmp', 'jpg', 'jpeg', 'pgm', 'png', 'ppm', 'tif', 'tiff', 'webp', 'exr'}
...
class ImagePathDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        path = self.files[i]
        # print(path)
        img = self.loadImage(str(path))
        if self.transforms is not None:
            img = self.transforms(img)
        # torch.nan_to_num(img, nan=0., posinf=16.)
        return img
...
def compute(path=["",""], num_workers=1, batch_size=1,dims=2048):
    ...
    fid_value = calculate_fid_given_paths(path, batch_size, device, dims, num_workers)
    return fid_value